...
- **`forge status --budget [--json] [--all]`** — a fleet-wide context-budget HUD reading each agent
  ...
  only when the tool's **exact statusline signature** matches — the co-occurring patterns a real
  statusline has and no log line can forge. Claude requires all three on one line: `ctx:NN%`, a
  sibling `5h:NN%`/`7d:NN%` gauge (`%` mandatory), and a whole-bracket versioned model token (model
  words, a space, then a version number: `[Opus 4.8:high]`, `[Fable 5]`); Codex: the `gpt-*` model
  token + middle-dot separators + both the `Context …% left` and `weekly …% left` gauges. A bare
  gauge phrase (`Context 5% left`, a `ctx:42%` in a log line, `[INFO] [priority:high] ctx:42%`,
  `[worker5] ctx:42% 5h:3`) matches none of that and is `n/a` by construction. Values: Claude
